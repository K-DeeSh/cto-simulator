...
export const ALL_EVENTS: GameEvent[] = [
  ...
  {
    id: 'geocoder_moon',
    title: 'Геокодер отправил груз на Луну',
    description: 'Клиент ввёл адрес "рядом с луной, второй кратер слева". Геокодер принял. Заказ создался. Курьер едет.',
    urgency: 'medium',
    ignore: {
      effects: { health: -10, reputation: -10 },
      successMessage: 'Курьер сам разобрался. Клиент живёт в Люблино.',
      riskChance: 0.4,
      riskEffects: { health: -10, money: -10 },
      riskMessage: 'Курьер провёл полдня на МКАД в поисках адреса. Смена сорвана.',
      microComment: 'Валидация адреса — не опция.',
    },
    delegate: {
      effects: { money: -5, techDebt: -5 },
      successMessage: 'Добавили санитайзер адресов. Лунные адреса теперь отклоняются.',
      microComment: 'Garbage in, garbage out. Классика.',
    },
  },
  ...
  {
    id: 'vladivostok_route',
    title: 'Оптимальный маршрут через Владивосток',
    description: 'Алгоритм решил: доставка из Бутово в Чертаново оптимальна через Владивосток. Курьер уже выехал.',
    urgency: 'high',
    ignore: {
      effects: { money: -15, morale: -10 },
      successMessage: 'Курьеры игнорируют маршруты и едут как знают. Кое-как работает.',
      riskChance: 0.35,
      riskEffects: { reputation: -15, money: -10 },
      riskMessage: 'Один курьер последовал маршруту. Сейчас где-то под Казанью.',
      microComment: 'TSP — нерешённая задача. Особенно с Владивостоком.',
    },
    delegate: {
      effects: { techDebt: 10, money: -10 },
      successMessage: 'Нашли регрессию в матрице расстояний. Откатили. Владивосток исключён.',
      riskChance: 0.25,
      riskEffects: { health: -15, techDebt: 5 },
      riskMessage: 'Откат сломал маршруты в Сибири. Там тоже недовольны.',
      microComment: 'Географические баги — самые живописные.',
    },
  },
  ...
  {
    id: 'wrong_entrance',
    title: 'Курьер и клиент ждут у разных подъездов',
    description: 'GPS показывает: курьер у первого подъезда, клиент у первого подъезда. Стоят уже 25 минут. Друг друга не видят. Оба правы.',
    urgency: 'medium',
    ignore: {
      effects: { reputation: -10, morale: -5 },
      successMessage: 'Разобрались по телефону через полчаса. Все устали.',
      microComment: 'Квантовая суперпозиция курьера и клиента.',
    },
    delegate: {
      effects: { money: -5, techDebt: -5 },
      successMessage: 'Добавили уточнение подъезда и фото точки встречи. Больше не теряются.',
      microComment: 'Last-metre problem — самая человечная часть логистики.',
    },
  },
  ...
  {
    id: 'pm_voice_47min',
    title: 'ТЗ в виде войса на 47 минут',
    description: 'Продакт прислал требования к новой фиче голосовым сообщением на 47 минут. Транскрипция недоступна. Дедлайн — пятница.',
    urgency: 'medium',
    ignore: {
      effects: { morale: -10, techDebt: 10 },
      successMessage: 'Команда реализовала то, что поняла из первых пяти минут.',
      riskChance: 0.45,
      riskEffects: { health: -10, morale: -10 },
      riskMessage: 'Сделали не то. На 45-й минуте войса был поворот сюжета.',
      microComment: 'TL;DR существует по причине.',
    },
    delegate: {
      effects: { morale: -5, money: -5 },
      successMessage: 'Попросили продакта написать письмом. Он обиделся, но написал.',
      microComment: 'Письменное ТЗ — это уважение к команде.',
    },
  },
  ...
  {
    id: 'todo_2016',
    title: '// TODO: убрать до релиза. Февраль 2016',
    description: 'В критичном сервисе тарификации нашли закомментированный блок логики с пометкой "TODO: убрать до релиза". Дата коммита: 14 февраля 2016. С тех пор было 1200 деплоев.',
    urgency: 'medium',
    ignore: {
      effects: { techDebt: 20 },
      successMessage: 'Оставили. Пусть живёт. Это уже часть архитектуры.',
      delayedEffect: {
        delay: 4,
        effects: { health: -15, morale: -5 },
        message: 'Тот самый TODO сломался. Никто не понимает что он делал.',
      },
      microComment: 'Восемь лет — хороший срок для TODO.',
    },
    delegate: {
      effects: { money: -10, morale: -5, techDebt: -10 },
      successMessage: 'Разобрались. Код был нужен. Задокументировали и оставили официально.',
      microComment: 'Иногда TODO — это честная документация.',
    },
  },
  ...
  {
    id: 'office_no_delivery',
    title: 'Наш офис — зона недоставки',
    description: 'Система определила собственный офис компании как "зону недоставки". Сотрудники не могут заказать обед через наш же сервис.',
    urgency: 'medium',
    ignore: {
      effects: { morale: -10, reputation: -5 },
      successMessage: 'Все заказывают через конкурентов. Неловко.',
      microComment: 'Сапожник без сапог — IT-издание.',
    },
    delegate: {
      effects: { morale: 5, money: -5 },
      successMessage: 'Нашли: офис попал в полигон ограничений по ошибке. Починили. Обед заказан.',
      microComment: 'Dogfooding начинается с того, что ты сам можешь пользоваться продуктом.',
    },
  },
  ...
  {
    id: 'prod_staging_swap',
    title: 'Прод и стейджинг поменялись местами',
    description: 'Junior перепутал конфиги при деплое. Прод работает на стейджинг-ресурсах с тестовыми данными. Стейджинг — на боевых. Клиенты видят тестовые заказы.',
    urgency: 'critical',
    ignore: {
      effects: { health: -20, reputation: -20 },
      successMessage: 'Само устаканилось. Несколько клиентов видели чужие заказы.',
      riskChance: 0.5,
      riskEffects: { reputation: -20, money: -15 },
      riskMessage: 'Клиент позвонил: "У меня в аккаунте чьи-то посылки из Екатеринбурга".',
      microComment: 'Environment confusion — классика жанра.',
    },
    delegate: {
      effects: { morale: -10, money: -5 },
      successMessage: 'Откатили. Внедрили защиту от смены окружения без подтверждения.',
      microComment: 'Деплой в прод должен быть скучным и предсказуемым.',
    },
  },
  ...
  {
    id: 'ceo_quantum',
    title: 'CEO хочет квантовую логистику',
    description: 'CEO вернулся с Давоса. Теперь хочет "квантовую оптимизацию маршрутов". Объяснить что это — не может, но видел в презентации McKinsey.',
    urgency: 'low',
    ignore: {
      effects: { reputation: -5 },
      successMessage: 'CEO прочитал новую статью. Теперь хочет нейроцепочку поставок.',
      microComment: 'Хайп-цикл Gartner — наш продуктовый роадмап.',
    },
    delegate: {
      effects: { money: -20, techDebt: 15, morale: -10 },
      successMessage: 'Сделали обычную оптимизацию и назвали её "квантовой". CEO доволен.',
      riskChance: 0.3,
      riskEffects: { reputation: -10, morale: -5 },
      riskMessage: 'CEO показал слайд инвесторам. Те спросили технических деталей. Неловко.',
      microComment: 'Quantum washing — новый green washing.',
    },
  },
